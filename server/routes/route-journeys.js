...
const Repo         = require('../repositories/JourneysRepository')
const feature      = get('journeys')
...
router.post('/new', verifyToken, checkRole('super_admin','admin'), auditLog, async (req, res, next) => {
  try {
    ...
    res.redirect('/cms/journeys/' + item.id)
  } catch(err) { next(err) }
})
...
router.post('/:id/edit', verifyToken, checkRole('super_admin','admin'), auditLog, async (req, res, next) => {
  try {
    ...
    res.redirect('/cms/journeys/' + req.params.id)
  } catch(err) { next(err) }
})
