...
const Repo         = require('../repositories/KpiEntriesRepository')
const feature      = get('kpi-entries')
...
router.post('/new', verifyToken, checkRole('super_admin','admin'), auditLog, async (req, res, next) => {
  try {
    ...
    res.redirect('/cms/kpi-entries/' + item.id)
  } catch(err) { next(err) }
})
...
router.post('/:id/edit', verifyToken, checkRole('super_admin','admin'), auditLog, async (req, res, next) => {
  try {
    ...
    res.redirect('/cms/kpi-entries/' + req.params.id)
  } catch(err) { next(err) }
})
