...
const Repo         = require('../repositories/SnapshotsRepository')
const feature      = get('snapshots')
...
router.post('/new', verifyToken, checkRole('super_admin','admin'), auditLog, async (req, res, next) => {
  try {
    ...
    res.redirect('/cms/snapshots/' + item.id)
  } catch(err) { next(err) }
})
...
router.post('/:id/edit', verifyToken, checkRole('super_admin','admin'), auditLog, async (req, res, next) => {
  try {
    ...
    res.redirect('/cms/snapshots/' + req.params.id)
  } catch(err) { next(err) }
})
