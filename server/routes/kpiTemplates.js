...
const Repo         = require('../repositories/KpiTemplatesRepository')
const feature      = get('kpi-templates')

// ── CMS PAGES ─────────────────────────────────────────────────────────────────

router.get('/', verifyToken, checkRole('super_admin'), checkProvince, async (req, res, next) => {
  try {
...
router.post('/new', verifyToken, checkRole('super_admin'), auditLog, async (req, res, next) => {
  try {
    ...
    res.redirect('/cms/kpi-templates/' + item.id)
  } catch(err) { next(err) }
})
...
router.post('/:id/edit', verifyToken, checkRole('super_admin'), auditLog, async (req, res, next) => {
  try {
    ...
    res.redirect('/cms/kpi-templates/' + req.params.id)
  } catch(err) { next(err) }
})
...
router.post('/:id/delete', verifyToken, checkRole('super_admin'), auditLog, async (req, res, next) => {
  try {
    ...
    res.redirect('/cms/kpi-templates')
  } catch(err) { next(err) }
})

// ── PUBLISH TOGGLE (API) ──────────────────────────────────────────────────────

router.post('/:id/publish', verifyToken, checkRole('super_admin'), auditLog, async (req, res, next) => {
  try {
...
router.get('/api', verifyToken, checkRole('super_admin'), checkProvince, async (req, res, next) => {
  try {
    const items = req.provinceFilter
      ? await Repo.findAll({ province_id: req.provinceFilter })
      : await Repo.findAll()
