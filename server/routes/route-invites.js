...
const Repo         = require('../repositories/InvitesRepository')
const feature      = get('invites')
...
router.post('/new', verifyToken, checkRole('super_admin'), auditLog, async (req, res, next) => {
  try {
    ...
    res.redirect('/cms/invites/' + item.id)
  } catch(err) { next(err) }
})
...
router.post('/:id/edit', verifyToken, checkRole('super_admin'), auditLog, async (req, res, next) => {
  try {
    ...
    res.redirect('/cms/invites/' + req.params.id)
  } catch(err) { next(err) }
})
