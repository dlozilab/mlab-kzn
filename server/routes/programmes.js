...
const Repo         = require('../repositories/ProgrammesRepository')
const feature      = get('programmes')
...
router.post('/new', verifyToken, checkRole('super_admin','admin'), auditLog, async (req, res, next) => {
  try {
    ...
    res.redirect('/cms/programmes/' + item.id)
  } catch(err) { next(err) }
})
...
router.post('/:id/edit', verifyToken, checkRole('super_admin','admin'), auditLog, async (req, res, next) => {
  try {
    ...
    res.redirect('/cms/programmes/' + req.params.id)
  } catch(err) { next(err) }
})
